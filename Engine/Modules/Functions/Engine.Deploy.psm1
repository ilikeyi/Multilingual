...
Function DeployTaskOffice
{
	...
	if (Test-Path -Path "$($env:SystemDrive)\Package\Office\Deploy.ps1" -PathType leaf) {
		Start-Process powershell -ArgumentList "-file $($env:SystemDrive)\Package\Office\Deploy.ps1" -Wait -WindowStyle Minimized
		return
	}
