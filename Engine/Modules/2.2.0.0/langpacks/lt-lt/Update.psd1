ConvertFrom-StringData -StringData @'
	# lt-LT
	# Lithuanian (Lithuania)

	ChkUpdate                 = Patikrinkite, ar nėra atnaujinimų
	UpdateServerSelect        = Automatinis serverio pasirinkimas arba pasirinktinis pasirinkimas
	UpdateServerNoSelect      = Pasirinkite pasiekiamą serverį
	UpdateSilent              = Atnaujinkite tyliai, kai yra naujinių
	UpdateClean               = Laisvalaikiu leiskite valyti senas versijas
	UpdateReset               = Iš naujo nustatykite šį sprendimą
	UpdateResetTips           = Kai atsisiuntimo adresas yra pasiekiamas, atsisiuntimas yra priverstinis ir atnaujinamas automatiškai.
	UpdateCheckServerStatus   = Patikrinkite serverio būseną ( galimos {0} parinktys )
	UpdateServerAddress       = Serverio adresas
	UpdatePriority            = Jau nustatytas kaip prioritetas
	UpdateServerTestFailed    = Nepavyko patikrinti serverio būsenos
	UpdateQueryingUpdate      = Užklausa dėl atnaujinimų...
	UpdateQueryingTime        = Tikrinama, ar yra naujausia versija. Prisijungimas užtruko {0} milisekundžių.
	UpdateConnectFailed       = Nepavyko prisijungti prie nuotolinio serverio, atnaujinimų tikrinimas nutrauktas.
	UpdateREConnect           = Ryšys nutrūko, bandoma dar kartą {0}/{1} ąjį kartą.
	UpdateMinimumVersion      = Atitinka minimalius atnaujinimo versijos reikalavimus, minimalią reikalaujamą versiją: {0}
	UpdateVerifyAvailable     = Patikrinkite, ar adresas yra
	UpdateDownloadAddress     = Atsisiųsti adresą
	UpdateAvailable           = Galima
	UpdateUnavailable         = Nėra
	UpdateCurrent             = Dabartinė versija
	UpdateLatest              = Naujausia versija
	UpdateNewLatest           = Atraskite naujas galimas versijas!
	UpdateSkipUpdateCheck     = Iš anksto sukonfigūruota politika neleisti automatiniams naujinimams paleisti pirmą kartą.
	UpdateTimeUsed            = Laikas, skirtas
	UpdatePostProc            = Po apdorojimo
	UpdateNotExecuted         = Neįvykdyta
	UpdateNoPost              = Po apdorojimo užduotis nerasta
	UpdateUnpacking           = Išpakavimas
	UpdateDone                = Sėkmingai atnaujinta!
	UpdateDoneRefresh         = Baigus atnaujinimą, atliekamas funkcijų apdorojimas.
	UpdateUpdateStop          = Atsisiunčiant naujinimą įvyko klaida ir atnaujinimo procesas buvo nutrauktas.
	UpdateInstall             = Ar norite įdiegti šį naujinimą?
	UpdateInstallSel          = Taip, aukščiau nurodytas naujinimas bus įdiegtas\nNe, naujinimas nebus įdiegtas
	UpdateNotSatisfied        = \n  Netenkinami minimalūs atnaujinimo programos versijos reikalavimai, \n\n  Minimali reikalinga versija: {0}\n\n  Prašome atsisiųsti dar kartą.\n\n  Atnaujinimų tikrinimas buvo nutrauktas.\n
	IsAllowSHA256Check        = Leisti SHA256 maišos patikrinimą
	GetSHAFailed              = Nepavyko gauti maišos, kad būtų galima palyginti su atsisiųstu failu.
	Verify_Done               = Patvirtinimas sėkmingas.
	Verify_Failed             = Patvirtinimas nepavyko, maišos neatitikimas.
	Auto_Update_Allow         = Leisti automatinius foninius atnaujinimų patikrinimus
	Auto_Update_New_Allow     = Aptikus naujus atnaujinimus, leisti automatinius atnaujinimus.
	Auto_Check_Time           = Valandos, intervalas tarp automatinių patikrinimų
	Auto_Last_Check_Time      = Paskutinio automatinio atnaujinimų patikrinimo laikas
	Auto_Next_Check_Time      = Ne daugiau kaip {0} valandų, kito patikrinimo laikas
	Auto_First_Check          = Atnaujinimų patikrinimas neatliktas, bus atliekamas pirmasis atnaujinimų patikrinimas
	Auto_Update_Last_status   = Paskutinio atnaujinimo būsena
	Auto_Update_IsLatest      = Jau yra naujausia versija.
'@